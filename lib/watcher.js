...
var PROXY_WATCHER_ETCD_SECURE = process.env.PROXY_WATCHER_ETCD_SECURE == 'false' ? false : true;
...
logger.infoGrey('env', 'PROXY_WATCHER_ETCD_CLUSTER_HOST - ' + PROXY_WATCHER_ETCD_CLUSTER_HOST);
logger.infoGrey('env', 'PROXY_WATCHER_ETCD_CLUSTER_PORT - ' + PROXY_WATCHER_ETCD_CLUSTER_PORT);
logger.infoGrey('env', 'PROXY_WATCHER_ETCD_PATH - ' + PROXY_WATCHER_ETCD_PATH);
logger.infoGrey('env', 'PROXY_WATCHER_CONFIG_PATH - ' + PROXY_WATCHER_CONFIG_PATH);
logger.infoGrey('env', 'PROXY_WATCHER_RELOAD_COMMAND - ' + PROXY_WATCHER_RELOAD_COMMAND);
...
var watchForChanges = function(){
	...
	watcher.on('change', function(modifiedNode){
		...
		if(modifiedNode.action == 'delete' || (newVal && !oldVal)){
			hasChanged = true;
		} else {
